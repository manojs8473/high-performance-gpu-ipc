...
int main() {
    std::cout << "Spin-Wait IPC Producer: Starting..." << std::endl;
    ...
    std::cout << "Spin-Wait IPC Producer: Device checks passed" << std::endl;
    ...
    std::cout << "Spin-Wait IPC Producer: IPC handle created successfully" << std::endl;
    ...
    std::cout << "Spin-Wait IPC Producer: Waiting for consumer..." << std::endl;
    
    // Spin-wait for consumer to connect (NO SLEEP!)
    while (!header->consumerReady) {
        // Pure spin-wait - no thread yield
    }
    
    std::cout << "Spin-Wait IPC Producer: Starting MAXIMUM SPEED image generation..." << std::endl;
    ...
    while (true) {
        // PURE SPIN-WAIT for previous frame to be processed (NO SLEEP!)
        while (!header->processComplete) {
            // Busy wait - maximum responsiveness
        }
        ...
        // Update header atomically - NO MEMORY TRANSFERS!
        ...
        if (duration.count() > 1000) {
            std::cout << "Spin-Wait IPC Producer FPS: " << (fpsCounter * 1000.0 / duration.count()) << std::endl;
